...
async function fetchOptions() {
    ...
    currencyArray = Object.entries(data.rates)
...
function createNewCurrency() {
  ...
  activeARR.push(currencyArray[index])
}

function convert(e) {
  let curValue = e.target.value
  let curName = e.target.parentNode.children[0].innerText
  const index = activeARR.map(e => e[0]).indexOf(curName);

  let diff = curValue/activeARR[index][1]

  activeARR.forEach((inp,index) => {
    if(curValue !== inputFields[index].value) { 
      inp[1] *= diff 
      inputFields[index].value = inp[1]
    }else{
      activeARR[index][1] = curValue
    }
})
